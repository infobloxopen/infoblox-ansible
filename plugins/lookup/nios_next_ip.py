...
- name: return next available IP address for network 192.168.10.0/24
  ansible.builtin.set_fact:
    ipaddr: |
      {{ lookup('infoblox.nios_modules.nios_next_ip', '192.168.10.0/24', provider={}) }}

- name: return next available IP address for network 192.168.10.0/24 in a non-default network view
  ansible.builtin.set_fact:
    ipaddr: |
      {{ lookup('infoblox.nios_modules.nios_next_ip', '192.168.10.0/24', network_view='ansible', provider={}) }}
...
- name: return next available IP address for network fd30:f52:2:12::/64
  ansible.builtin.set_fact:
    ipaddr: |
      {{ lookup('infoblox.nios_modules.nios_next_ip', 'fd30:f52:2:12::/64',
          provider={'host': 'nios01', 'username': 'admin', 'password': 'password'}) }}
"""
